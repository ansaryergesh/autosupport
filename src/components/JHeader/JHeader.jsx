...
const JHeader = ({isQuestion = true, questionInfo = initialQuestionDto}) => {
    const questionCategory = isQuestion && getCategoryByLangKey(questionInfo.categorie?.categorieContents, getLocale().toUpperCase());
    const questionCategoryTitle = questionCategory?.name;
    const questionCategoryId = questionCategory?.id;

    const questionByKey = isQuestion && getCategoryByLangKey(questionInfo.questionContents, getLocale().toUpperCase());
    const questionTitle = questionByKey?.title;
    const questionDescription = questionByKey?.stepDescription;

    return (
        <div>
            {questionInfo !== {} && isQuestion && <Breadcrumb
            ...
            />}
                <TypographyHead className={'headerText'} type={TypoGraphyType.HEADER} content={!isQuestion ? 'Главная' : questionTitle} />
