...
const ImageList = ({ displayFileList, fileList, setFileList, handlePreview, handleRemove}) => {
  ...
  const moveItem = (firstId, secondId) => {
    const firstIndex = fileList.findIndex(item => item.uid === firstId);
    const secondIndex = fileList.findIndex(item => item.uid === secondId);
  ...
  const RenderImage = (item, index) => {
    const [, ref] = useDrag({
      type: 'IMAGE_ITEM',
      item: {id: item.item.uid},
    });

    const [, drop] = useDrop({
      accept: 'IMAGE_ITEM',
      drop: (draggedItem) => {
        if (item.item.uid !== draggedItem.id) {
          moveItem(draggedItem.id, item.item.uid);
        }
      },
    });
